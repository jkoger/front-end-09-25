...
function GiftCards() {
  ...
  return (
    <div>
      ...
      {!showInput && amount > 0 && (
        <button onClick={() => setShowInput(true)}>Valin ise summa</button>
      )}
      {showInput && (
        <>
          <input
      ...
      {!showInput && customAmount && amount === 0 && (
        <button className="amount-active" onClick={() => setShowInput(true)}>
          {customAmount} eur
        </button>
      )}
      <br /> <br />
      ...
      <div>
        Kinkekaardi kogusumma on: {(amount || customAmount) * quantity} eur{" "}
      </div>
    </div>
  );
}
