...
export default function Dropdown({handleSelect, options, header}) {

    const [selected, setSelected] = useState("");

    function setSelectedHandler(value){
        setSelected(value);
        handleSelect(value, header);
    }
  return (
        ...
        <div className="py-1">
         {options.map (option =>  <MenuItem key ={option} >
            ...
            onClick ={() => setSelectedHandler(option)}
              className=" dropdown-option block px-4 py-2 text-sm text-gray-300 data-focus:bg-white/5 data-focus:text-white data-focus:outline-hidden"
            >
              {option}
            </span>
          </MenuItem>
